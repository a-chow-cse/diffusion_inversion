...
class EmbModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.emb = nn.Linear(in_features=self.num_emb,
                             out_features=self.emb_ch*self.num_tokens, bias=False)

    def forward(self, x):
        emb = self.emb(x)
        emb = emb.view(-1, self.num_tokens, self.emb_ch)
        return emb


class EmbPipeline(DiffusionPipeline):
    r"""
    ...
    def __init__(
        ...
        self.avg_uncond_embeddings = avg_uncond_embeddings
    ...
    def _encode_prompt(self, prompt, device, num_images_per_prompt, do_classifier_free_guidance, emb_noise, negative_prompt):
        r"""
        ...
        if emb_noise > 0.0:
            noise = torch.randn(text_embeddings.shape, dtype=text_embeddings.dtype,
                                device=text_embeddings.device) * emb_noise
            text_embeddings = text_embeddings + noise

        # get unconditional embeddings for classifier free guidance
        if do_classifier_free_guidance:
            if self.uncond_embeddings is None:
                if self.avg_uncond_embeddings:
                    emb_weight = self.emb.emb.weight.detach()
                    uncond_embeddings = emb_weight.mean(
                        dim=1).view(1, seq_len, -1)
                else:
    ...
    def __call__(
        self,
        prompt: Union[np.ndarray, torch.Tensor],
        ...
        if isinstance(prompt, np.ndarray):
            ...
            prompt = torch.from_numpy(prompt).to(
                self.unet.conv_in.weight.dtype).to(self._execution_device)
        ...
        text_embeddings = self._encode_prompt(
            prompt, device, num_images_per_prompt, do_classifier_free_guidance, emb_noise, negative_prompt
        )
